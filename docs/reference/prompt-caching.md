...
- DeepSeek OpenAI-compatible Completions models default to `compat.disableBoundaryAwareCache: true`. OpenClaw keeps the stable system-prompt prefix in the system message and appends the dynamic cache-boundary suffix as a trailing system-role carrier after the full conversation history, so the stable system-prompt prefix remains reusable across turns for prefix-matching provider caches. Xiaomi users can opt in by setting `compat.disableBoundaryAwareCache: true` manually.
