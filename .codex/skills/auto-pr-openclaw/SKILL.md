...
description: Prepare and maintain contributor pull requests for openclaw/openclaw issues with repo-policy intake, canonical per-issue worktrees, executable validation checks, durable PR-body evidence, and automatic publication when all pre-push gates pass. An explicit existing-PR maintenance request authorizes the validated publication without redundant confirmation; human decisions remain for blocked, local, policy, uncertain, or exceptional attempts. An agent may bypass only evidence-backed external blockers. Use when Codex is asked to handle an OpenClaw issue, screen local or remote candidate issues, find easy-to-merge PR opportunities, prepare or update an OpenClaw PR, respond to ClawSweeper/Codex review, debug PR CI, or request ClawSweeper re-review for openclaw/openclaw.
---

# OpenClaw PR Workflow

For scheduled maintenance of already-published PRs, read
`references/scheduled-maintenance.md` first. It defines intake, resume, bounded
repair, and completion behavior; use the shared validation and publication
gates below only for the selected maintenance path. Candidate mining and new-PR
scouting do not apply to this mode.

For scheduled discovery and publication of new PRs, read
`references/scheduled-candidates.md` first. It bounds exploration and places
live duplicate screening and proof feasibility before expensive setup. Keep
the normal new-PR validation and publication gates.

## Core Rules

Use this skill for `openclaw/openclaw` contributor work. Keep the process conservative and evidence-first.

- Treat GitHub writes as gated by default: always generate and inspect the publication gate summary before pushing, creating/updating a PR, or editing the PR body. When `automaticPublication.eligible` is true and `blockers` is empty, invoke the automatic publisher without a second confirmation. When blockers remain, inspect `blockerDetails` and `agentExternalBypass`: if every blocker is explicitly eligible and high-confidence evidence shows the cause is external, create the evidence-backed agent judgment described in `references/evidence-receipts.md` and invoke the automatic publisher with it; otherwise show the gate packet and request human confirmation. Comments, review requests, and the clean rebase-only path retain their separate rules.
- An explicit request to maintain or update an already-published PR, including review-driven wording such as “根据评论优化 PR #120860”, is the human authorization for that requested branch/body update. Once the publication gate is clear, do not ask for a duplicate confirmation merely because the task included conflict resolution or uses the normal publisher. This authorization does not bypass a blocker and does not authorize comments, review requests, ClawSweeper re-review, or work outside the requested PR; review/report/draft/local-only requests do not authorize a GitHub write.
...
- For candidate speed and quality, run `scripts/openclaw-candidate-scout.sh` before implementation, use `scripts/openclaw-duplicate-check.sh` only for unpublished candidates, then use `scripts/openclaw-proof-receipt.sh`, `scripts/openclaw-candidate-score.sh`, and `scripts/openclaw-gate-summary.sh` when a workflow exists. The gate summary is the read-only/local-output publication decision: it authorizes the automatic publisher when it has no blockers, or when every remaining blocker is explicitly eligible for an evidence-backed external-cause judgment, and remains bound to the current workflow state.
...
- After the publication gate passes, publish branches through the workflow's selected GitHub account using `scripts/publish-openclaw-pr.mjs --auto-if-ready --gate-summary <gate-summary.json>` unless the script is missing or fails for a concrete reason you report. If the gate has blockers, use the normal publisher only after explicit human confirmation unless every blocker is allowlisted and an evidence-backed agent judgment validates the external cause. The publisher owns branch push, REST PR create/update, exact PR body verification, and maintainer-edit verification. Do not hand-roll `git push` plus `gh pr create/edit` when the publisher is available. New PRs are ready for review by default; create a draft only when the user explicitly asks for draft, the PR is intentionally incomplete, or an explicitly bypassed gate leaves review premature. Publishing or updating a PR never includes an automatic `@clawsweeper re-review`.
...
9. When the user picks a candidate, or an authorized automatic task selects one within its scope, switch to the normal PR workflow below. Do not wait for another selection in an already-authorized scheduled publication task.
...
8. When the user picks a candidate, or an authorized automatic task selects one within its scope, switch to issue intake and normal PR workflow. Do not stop at a shortlist when implementation and publication were requested.
...
3. **Executable local checks**
   ...
   - Preflight may reuse successful heavy checks only when its fingerprint matches the current HEAD, pinned validation base, package and lockfile content, selected lanes, targeted planner implementation, toolchain, platform, and relevant execution environment. The check lane (lint/format/types) is keyed on the committed diff content and lint-relevant config blobs instead of the commit SHA, so a rebase, amend, or squash that preserves file contents keeps a passing check lane cached; the focused-test fingerprint is keyed on the HEAD tree hash, so content-preserving history rewrites also reuse passing tests while a rebase onto advanced main correctly misses. Latest observed main is deliberately excluded from those fingerprints; merge risk is recomputed separately on every run.
   - During iterate-and-fix loops (reruns after code changes or conflict fixes before the publication attempt), prefer `openclaw-preflight.sh --profile targeted` as the fast advisory lane; the check-lane and focused-test caches keep repeated iterations cheap. Reserve the default `auto` profile for the publication attempt so broad or cross-surface diffs still escalate to `changed`.
   ...
   **Clean rebase-only fast path:** If the user explicitly asks to rebase or catch up an existing PR, fetch once, pin the target SHA, and run `git rebase <target-sha>`. If Git reports any conflict, stop immediately; do not resolve it under this path. After a conflict-free rebase, run `scripts/openclaw-rebase-only-check.sh --workflow <outputs>/workflow.json --target <pinned-rebase-target-sha>`. This lightweight check compares the ordered stable patch-id series before and after the rebase and verifies identity, clean state, target ancestry, PR-head ownership, the unchanged remote head, maintainer edit access, and that the canonical PR body does not pin the pre-rebase head SHA. If it passes, immediately publish with `scripts/publish-openclaw-rebase-only.mjs --workflow <outputs>/workflow.json --target <pinned-rebase-target-sha> --push-remote <remote>`; do not run preflight, gate summary, tests, lint, type checks, `git diff --check`, or ask for a second confirmation. If the check refuses because the body pins a stale head SHA, do not push: refresh the body with `scripts/refresh-pr-body-sha.sh --workflow <outputs>/workflow.json`, rerun `scripts/validate-pr-body.mjs --workflow <outputs>/workflow.json` (body-only edit), regenerate the publication gate, and use `scripts/publish-openclaw-pr.mjs --auto-if-ready` when it is clear; if it remains blocked, use the normal publisher after approval. The rebase-only receipt's patch equivalence is what makes the SHA transfer to the new head honest. Do not use this path for conflict resolution, review-driven code edits, proof/body changes beyond the deterministic SHA refresh, new PR creation, or any case that needs ClawSweeper re-review.

   **Conflict-resolution fast path:** Start the rebase through `scripts/openclaw-conflict-rebase.sh --phase start` before Git changes the branch. A conflict exit is expected; resolve only the recorded files. If a later `rebase --continue` stops again, run `--phase record` before resolving it. Merge-containing PR history is supported through stable net-patch equivalence. Create a small JSON validation plan containing only commands that rebuild a conflicted generated asset, exercise focused affected tests, or check a directly affected package boundary. When conflict/CI investigation requires a small maintenance edit outside the conflict files, list it in `allowedFiles` and add the matching focused command; finish only after those edits are complete. Finish with `--phase finish --plan <plan.json>`. The receipt permits up to eight explicitly scoped files across surfaces, rejects high-risk paths, and rejects any patch drift outside that scope. Generated conflicts require a rebuild command and non-document changes require a focused-test command. If it passes, run `openclaw-preflight.sh --profile conflict`, inspect the publication gate, and publish when it is eligible and blocker-free if the user explicitly requested maintenance of this existing PR. Ask for confirmation only when the request did not authorize a remote update or a blocker remains. If it refuses, stop publication and inspect the exact invariant. Correct an evidence-backed error within the authorized scope and regenerate the receipt; preserve the failed receipt. If scope or semantic drift remains, report the precise decision and estimated heavier lanes. Do not start normal `auto`/`targeted`/`changed` validation without explicit user approval.

4. **Pre-push publication gate**
   - When a PR or candidate branch carries more than one commit and every commit between the pinned validation base and HEAD is authored and committed by the selected account, squash the history into one commit before preflight: run `node scripts/openclaw-squash-pr-commits.mjs --workflow <outputs>/workflow.json` (use `--dry-run` to preview). The script refuses when any commit has a foreign author or committer, when a remote PR contains commits from other logins, or when the worktree is dirty, and it preserves the exact tree, so the preflight check-lane and focused-test caches stay valid. Preflight must run (or rerun) after squashing so the receipt binds to the new HEAD.
   - Run `scripts/openclaw-gate-summary.sh --workflow <outputs>/workflow.json` and use the generated `gate-summary.json`/`gate-summary.md` as the publication receipt. It records the exact conditions that determine whether a second confirmation is needed.
   - Verify every workflow commit uses the selected account's author and committer identity. If no account profile was selected, verify every `sunlit-deng` commit uses author and committer email `yang.jiajun1@xydigit.com`. A mismatch remains a blocker; fix it before publication. Do not rewrite other contributors' authored commits merely to change their author email unless the selected-account rule is intentionally being applied to this workflow.
   - If `automaticPublication.eligible` is true and `blockers` is empty, publish immediately with `scripts/publish-openclaw-pr.mjs --auto-if-ready --gate-summary <outputs>/gate-summary.json`, passing the reviewed `--title` and `--head` for a new PR. Do not request a second confirmation.
   - If blockers remain, inspect `blockerDetails`. When `agentExternalBypass.eligible` is true and the agent can establish high confidence that every listed blocker is caused by upstream state, external infrastructure, tooling/environment, or unrelated CI, write the bound judgment receipt from `references/evidence-receipts.md` and add `--agent-judgment <outputs>/agent-publication-judgment.json` to the automatic publisher. Do not classify a local change failure, duplicate uncertainty, identity/body/worktree issue, maintainer-access issue, target mismatch, or remote lease issue as external.
   - If the judgment cannot account for every blocker with evidence, show the user the diff, identity, checks, body hash, proof, maintainer-edit status, and unresolved risk, then request explicit confirmation before using the normal publisher with the displayed HEAD/body hashes. A blocked gate is not an automatic workflow-rule bypass.
   - When the PR already has a ClawSweeper review, show its current verdict and unresolved blocking findings when the gate is blocked. Neither automatic nor manually confirmed publication authorizes `@clawsweeper re-review`; re-review requires a separate explicit request.
   - The publisher revalidates the current or gate-bound HEAD/body, pushes through the selected account, creates or updates the PR through REST, and verifies the exact remote body, target, and maintainer edit status. New PRs should be ready for review by default; use draft only when explicitly requested or intentionally incomplete. For an explicit workflow-rule bypass, use `--allow-workflow-rule-bypass --workflow-rule-bypass-reason "<user reason>"` and record the bypassed checks.
   ...
   - Conflict-resolved rebases use the publication gate; an explicit existing-PR maintenance request supplies the normal publication authorization once the gate is clear, so no duplicate conversational confirmation is needed. Clean rebase-only maintenance bypasses this section and uses `scripts/publish-openclaw-rebase-only.mjs`, which requires a current passing patch-equivalence receipt, uses `--force-with-lease`, re-checks the PR head owner/ref and maintainer edit access, pushes only the branch, and verifies the remote PR head SHA. It intentionally does not read or write the PR body.
   - For an existing PR, compare the live remote `headRefName` and the workflow's `headRef` with the target PR; do not compare them to the canonical local worktree branch name (`sunlit/pr-<number>`), which is intentionally different. If a blocker-free automatic publish is rejected only because of that local-versus-remote naming difference, re-read the live owner, remote ref, head SHA, and `maintainerCanModify`, then use the normal publisher bound to the gate's `--approved-head` and `--approved-body-sha`. Never use the broad workflow-rule bypass for this expected naming difference, and retain all lease and final remote-integrity checks.
...
Generate the publication gate summary:
...
check refuses (local edit only; then re-validate the body and regenerate the
publication gate):
