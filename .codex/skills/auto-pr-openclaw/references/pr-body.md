...
- **Tie proof to the exact head.** Include the tested head SHA in the Evidence transcript. For a high-confidence comparison, run the same command, production entrypoint, input, and loopback/fixture dependency on `validationBaseSha` and the exact PR head; label the two outputs `base` and `head`. After a rebase, the head SHA changes: refresh the exact-head SHA to the new head before push (the rebase-only gate refuses while the old head SHA is pinned, and `refresh-pr-body-sha.sh` rewrites it deterministically). The refresh is honest because the rebase-only receipt proves the ordered patch series is identical. Base-SHA references are historical records of where the base-side run happened and should be left as recorded.
